...
def test_default_terminal_reward_is_terminal_only(
...
def test_reset_uses_seeded_deterministic_progression_and_initial_legality(mission) -> None:
    ...
    alternate_step_observation, _, _, _, _ = env.step(select_unit_a_id)
    ...
    assert first_reset_info["legal_action_ids"] == [0, 1]
    assert sum(first_reset_info["legal_action_mask"]) == 2
...
def test_step_rejects_invalid_action_ids_without_advancing_state(mission) -> None:
    ...
    illegal_in_context_action_id = env.action_catalog.encode(
        ChooseOrderExecutionAction(choice=OrderExecutionChoice.FIRST_ORDER_ONLY),
    )
    valid_action_id = env.action_catalog.encode(
        SelectBritishUnitAction(unit_id="rifle_squad_a"),
    )

    with pytest.raises(InvalidActionIdError, match="outside the Mission 1 catalog range"):
...
@pytest.mark.parametrize(
    ("seed", "actions", "expected_outcome", "expected_reward"),
    [
        (0, _victory_actions(), "victory", 1.0),
        (3, _defeat_actions(), "defeat", -1.0),
    ],
    ids=["victory", "defeat"],
)
def test_step_runs_full_episode_with_terminal_reward_and_no_default_truncation(
    mission,
    seed: int,
    ...
    encoded_actions = tuple(env.action_catalog.encode(action) for action in actions)
    ...
    for index, action_id in enumerate(encoded_actions, start=1):
        ...
        if is_final_step:
            ...
            assert truncated is False
            assert observation["terminal_outcome"] == expected_outcome
            assert info["terminal_outcome"] == expected_outcome
...
def test_step_uses_truncated_only_for_external_step_limit(mission) -> None:
    env = Mission1Env(mission, max_steps=1)
    select_unit_a_id = env.action_catalog.encode(
        SelectBritishUnitAction(unit_id="rifle_squad_a"),
    )

    env.reset(seed=0)
    observation, reward, terminated, truncated, info = env.step(select_unit_a_id)
