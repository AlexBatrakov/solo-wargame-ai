...
2. `Phase Master Thread`
3. `Delivery Thread`
4. `Optional External Audit Thread`

Not every phase needs all four at once.
...
- deciding when a new Phase Master Thread should be opened.

The Super Master Thread should not be used for routine implementation.

### Phase Master Thread

One thread per phase.

Responsibilities:

- refine the phase plan,
- maintain the active phase packet,
- define delivery packages,
- generate prompts for Delivery Threads,
- accept/reject completed delivery packages,
- update phase status and closeout records,
- hand off to the next macro-step.

The Phase Master Thread should normally stay out of code implementation.
...
- report checkpoints back to the user for the Phase Master Thread,
...
- `2` to `4` Delivery Threads per phase,
...
Each Phase Master Thread should maintain a compact active phase packet.
...
The phase packet should contain only the active essentials:
...
Implementation threads should start from the phase packet first, then read only
...
The Phase Master Thread should dispatch the package with:
...
### When to return to the Phase Master Thread

Return to the Phase Master Thread when:
...
Do not bounce back to the Phase Master Thread after every tiny substep.
...
## Default thread budget per phase

Preferred:

- `1` Phase Master Thread
- `2` to `4` Delivery Threads
- `0` or `1` optional external audit threads

## Acceptance states for a delivery package

The Phase Master Thread should respond to a finished Delivery Thread report with
